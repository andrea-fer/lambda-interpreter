...
export default class CallByValueLambdaVisitor extends LambdaInterpreterVisitor {
	...
	visitApplication(ctx) {
        ...
        let brackets = false;
        if(leftChild.getText() == '(' && ctx.getChild(1) != null) {
        ...
        // get rid of brackets if there is no other term inside application
        ...
        if(!(leftChild instanceof LambdaParser.AbstractionContext) && !(leftChild.getChild(0) instanceof LambdaParser.ApplicationContext)) {
            ...
            for(let [key, value] of this.definitions) {
                if(left.includes(key)) {
                    ...
                    if(leftChildText.charAt(0) == '(' && leftChildText.charAt(leftChildText.length - 1) != ')') {
        ...
        while(leftChild instanceof LambdaParser.ApplicationContext 
            ...
            console.log("before visiting left child (", super.getTreeText(leftChild), ") in last term, '", this.terms[this.terms.length - 1], "'")
            leftChild = this.visit(leftChild);
            console.log("after visiting left child (", super.getTreeText(leftChild), ") in last term, '", this.terms[this.terms.length - 1], "'")
            if(leftChild == null) {
                return null;
            }
        ...
        while(rightChild.getChild(0) instanceof LambdaParser.ApplicationContext) {
            // if the rightChild consists of more than 1 application, select the first
            if(rightChild.getChild(0) != null && rightChild.getChild(0).getText() != '('
            && rightChild.getChild(0).getChild(0) instanceof LambdaParser.ApplicationContext) {
                //console.log("choosing first child ", super.getTreeText(rightChild), " --> ", super.getTreeText(rightChild.getChild(0)))
                rightChild = rightChild.getChild(0);
            }

            //console.log(super.getTreeText(rightChild.getChild(0)), " is an application")

            // if the rightChild is Variable, don't apply reduction
            if((rightChild.getChild(0) != null && rightChild.getChild(0).getChild(0) != null)
                && rightChild.getChild(0).getChild(0).getText() == '(' && rightChild.getChild(0).getChild(1).getChild(0).getChild(0) == null
                && rightChild.getChild(0).getChild(2) == null
                || rightChild.getChild(0).getChild(0).getText() != '(' && rightChild.getChild(0).getChild(0).getChild(0) == null
                && rightChild.getChild(0).getChild(1) == null) {
                //console.log("!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
            ...
            console.log("before visiting right child (", super.getTreeText(rightChild.getChild(0)), ") in last term, '", oldTerm, "'")
            rightChild = this.visit(rightChild.getChild(0));
            console.log("after visiting right child (", super.getTreeText(rightChild), ") in last term, '", oldTerm, "'")
            if(rightChild == null) {
                return null;
            }
            value = super.getTreeText(rightChild);
            ...
            //console.log("NEW CTX:= ", oldTerm.replace(super.getTreeText(oldRightChild), value));
            ...
            newCTX = newCTX.replace(newCTX, value);
